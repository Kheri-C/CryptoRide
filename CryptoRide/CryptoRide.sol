...
contract CryptoRide{  
    ...
    function set(uint _num) public{
        prueba memory myPrueba;
        myPrueba.num = _num;
        pruebas.push(myPrueba);
    }

    function get() public view returns(uint){
        return pruebas[0].num;
    }
    ...
    function publishRide(uint16 _id, string calldata _fromTime, string calldata _toTime, uint _baseCost, uint _biddingTime, uint16 _avSeats) public {
        ...
        //}
        ride memory myRide;
        myRide.driver = payable(msg.sender); // Driver
        myRide.available = true; // Disponibilidad
        myRide.payed = false; // Pagada
        myRide.fromTime = _fromTime; // Tiempo de inicio
        myRide.toTime = _toTime; // Tiempo de fin
        myRide.baseCost = _baseCost; // Costo minimo
        myRide.avSeats = _avSeats; // Asientos
        myRide.biddingTime = _biddingTime; // Tiempo para hacer bids
        myRide.biddingEndTime = (block.timestamp + _biddingTime); // Tiempo de fin de bids
